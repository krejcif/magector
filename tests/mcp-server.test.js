...
async function main() {
  ...
  try {
    ...
    log(tools.length === 36 ? 'PASS' : 'FAIL', `tools/list returns 36 tools`, `got ${tools.length}`);
    ...
    const stderrClean = client.stderrOutput
      ...
      .replace(/Magector MCP server connected \(warming up\.\.\.\)\n/g, '')
      .replace(/Serve process ready \(primary\)\n/g, '')
