...
- `cleanup()` removes the reindex PID file synchronously right after sending the kill signal, without waiting for the child to actually exit — a new session starting in that narrow window could spawn a second `magector-core index` writing to the same temp path. Low-probability progress loss, not corruption; needs waiting for the child's exit (or a file lock) to close properly.
